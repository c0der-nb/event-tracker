...
export default async function EventDetailPage({ params }) {
    const apiResponse = await fetch(`https://qevent-backend.labs.crio.do/events/${params.eventId}`);
            ...
            <div className="flex flex-col h-[auto] w-[80%] gap-y-20">
                <div className="flex flex-col w-full">
                    <div className="w-full flex flex-row justify-center">
                        <img className="h-[290px] w-[490px] object-cover" src={event.image} alt={event.name} />
                    </div>
                ...
                <div className="flex flex-col w-full gap-y-2">
                    <div className="flex flex-row gap-x-4">
                        {event.tags.map(tag => (<Tag text={tag} />))}
                    </div>
                    ...
                    <div className="flex flex-row justify-between w-full">
                        ...
                        <button className="h-[30px] w-[100px] text-center bg-red-500 text-white border-none text-sm rounded">Buy Tickets</button>
                    </div>
                </div>
            </div>
        </div>
